...
class FetchRecommendedSimilarListUseCase(
    ...
    BaseBusyObservable<FetchRecommendedSimilarListUseCase.Listener>() {
    interface Listener {
        fun onRecommendedSimilarMoviesSuccess(movies: MoviesResponse)
        fun onRecommendedSimilarMoviesFailure(msg: String)
    }
    ...
    fun fetchRecommendedSimilarMoviesUseCase(
        groupType: GroupType,
        page: Int,
        movieId: Int
    ) {
    ...
    private fun notifyFailure(error: String) {
        uiThreadPoster.post {
            listeners.forEach {
                it.onRecommendedSimilarMoviesFailure(error)
            }
            becomeNotBusy()
        }
    }
    ...
    private fun notifySuccess(movies: MoviesResponse) {
        uiThreadPoster.post {
            listeners.forEach {
                it.onRecommendedSimilarMoviesSuccess(movies)
            }
            becomeNotBusy()
        }
    }
